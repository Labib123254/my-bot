...
def init_db():
    try:
        ...
        # টাস্কের status ট্র্যাক করার জন্য নতুন column (pending/approved/rejected)
        # যাতে /rejecttask দিয়ে ভুয়া/ভুল কাজ বাতিল করা যায় এবং ভবিষ্যতে
        # একই টাস্ক দুইবার প্রসেস না হয়
        cur.execute('''
...
def get_all_tasks_file(message):
    ...
    # /gettasks pending  -> শুধু এখনো রিভিউ না হওয়া টাস্কগুলো দেখাবে
    # /gettasks          -> সব টাস্ক (pending/approved/rejected) দেখাবে
    ...
    if status_filter in ('pending', 'approved', 'rejected'):
        cur.execute("SELECT * FROM submitted_tasks WHERE status = %s ORDER BY id ASC", (status_filter,))
    else:
    ...
    if not tasks_rows:
        bot.reply_to(message, "⚠️ কোনো টাস্ক পাওয়া যায়নি।")
        return

    file_content = "=== ALL SUBMITTED TASKS ===\n\n"
    for item in tasks_rows:
        file_content += (
            f"[Task ID: {item['id']}] Status: {item['status']}\n"
...
def user_history(message):
    ...
    if not tasks_rows:
        bot.send_message(message.chat.id, "📌 **জমা দেওয়া কাজের তালিকা:** কোনো কাজ পাওয়া যায়নি।", parse_mode="Markdown")
    else:
        task_text = f"📌 **জমা দেওয়া কাজের তালিকা ({len(tasks_rows)} টি):**\n\n"
        for item in tasks_rows:
            task_text += (
                f"[Task ID: {item['id']}] Status: {item['status']}\n"
                f"    {item['task_type']} | Name: {item['acc_name']}\n"
                f"    সময়: {item['created_at']}\n"
                f"----------------------------------------\n"
            )
        # টেলিগ্রামের ৪০৯৬ ক্যারেক্টার লিমিটের কারণে বড় হলে ভাগ করে পাঠানো হচ্ছে
        for i in range(0, len(task_text), 3500):
            bot.send_message(message.chat.id, task_text[i:i+3500], parse_mode="Markdown")

    if not withdraw_rows:
        bot.send_message(message.chat.id, "💰 **উইথড্র হিস্টরি:** কোনো উইথড্র পাওয়া যায়নি।", parse_mode="Markdown")
    else:
        ...
        for i in range(0, len(withdraw_text), 3500):
            bot.send_message(message.chat.id, withdraw_text[i:i+3500], parse_mode="Markdown")
...
def reject_tasks(message):
    ...
    # ফরম্যাট: /rejecttask <id1,id2,id3,...> <কারণ>
    parts = message.text.split(maxsplit=2)
    if len(parts) < 3:
        bot.reply_to(
            message,
            "⚠️ নিয়ম: `/rejecttask <id1,id2,id3,...> <কারণ>`\n\n"
            "উদাহরণ: `/rejecttask 101,102,105 আপনার কুকিজ সঠিক ছিল না`\n\n"
            "Task ID গুলো `/gettasks` কমান্ড থেকে পাবেন।",
            parse_mode="Markdown"
        )
        return
    ...
    if not task_ids:
        bot.reply_to(message, "⚠️ কোনো সঠিক Task ID পাওয়া যায়নি। কমা (,) দিয়ে আলাদা করে ID দিন।")
        return

    bot.reply_to(message, f"⏳ {len(task_ids)} টি টাস্ক প্রসেস করা হচ্ছে, একটু সময় লাগতে পারে...")

    threading.Thread(target=_do_reject_tasks, args=(message.chat.id, task_ids, reason), daemon=True).start()

def _do_reject_tasks(admin_chat_id, task_ids, reason):
    conn = get_db_connection()
    cur = conn.cursor()

    rejected = []       # (task_id, user_id)
    not_found = []
    already_processed = []

    for task_id in task_ids:
        cur.execute("SELECT * FROM submitted_tasks WHERE id = %s", (task_id,))
        row = cur.fetchone()
        ...
        if row['status'] != 'pending':
            already_processed.append(task_id)
            continue

        # টাস্কটি rejected হিসেবে মার্ক করা হলো (হিস্টরির জন্য রেখে দেওয়া হচ্ছে, মুছে ফেলা হচ্ছে না)
        cur.execute("UPDATE submitted_tasks SET status = 'rejected' WHERE id = %s", (task_id,))
        ...
        rejected.append((task_id, row['user_id']))
    ...
    sent = 0
    failed_send = 0
    for task_id, uid in rejected:
        try:
            bot.send_message(
                uid,
                f"❌ **আপনার একটি কাজ বাতিল করা হয়েছে!**\n\n"
                f"📝 **কারণ:** {reason}\n\n"
    ...
    summary = (
        f"✅ **রিজেক্ট প্রসেস সম্পন্ন!**\n\n"
        f"🚫 বাতিল হয়েছে: {len(rejected)} টি টাস্ক\n"
        f"📨 মেসেজ পাঠানো হয়েছে: {sent} জনকে\n"
    )
    if failed_send:
        summary += f"⚠️ মেসেজ পাঠানো যায়নি: {failed_send} জনকে\n"
    if already_processed:
        summary += f"ℹ️ আগেই প্রসেস হয়ে গিয়েছিল (স্কিপ): {', '.join(map(str, already_processed))}\n"
    if not_found:
