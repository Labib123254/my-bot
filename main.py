...
def approve_tasks_bulk(message):
    ...
    threading.Thread(
        ...
        args=(message.chat.id, user_ids, amount, task_type_names[task_type_key]),
        daemon=True
    ).start()

def _do_approve_tasks_bulk(admin_chat_id, user_ids, amount, task_type_display):
    approved = []
    not_found = []
...
def reject_tasks(message):
    ...
    # ফরম্যাট: /rejecttask <user_id1,user_id2,user_id3,...> <কারণ>
    parts = message.text.split(maxsplit=2)
    if len(parts) < 3:
        bot.reply_to(
            message,
            "⚠️ নিয়ম: `/rejecttask <user_id1,user_id2,...> <কারণ>`\n\n"
            "উদাহরণ: `/rejecttask 7236432836,8757506241 আপনার কুকিজ সঠিক ছিল না`",
            parse_mode="Markdown"
        )
        return
    ...
    threading.Thread(target=_do_reject_tasks, args=(message.chat.id, user_ids, reason), daemon=True).start()

def _do_reject_tasks(admin_chat_id, user_ids, reason):
    rejected = []
    not_found = []
