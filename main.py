...
def init_db():
    try:
        ...
        # Facebook টাস্কে UID নেওয়ার পর, কুকিজ চাওয়ার আগ পর্যন্ত UID টা
        # সাময়িকভাবে রাখার জন্য কলাম (দুই ধাপ - প্রথমে UID, তারপর কুকিজ)
        cur.execute('''
            ALTER TABLE users ADD COLUMN IF NOT EXISTS temp_fb_uid TEXT DEFAULT NULL
        ''')
...
def is_valid_fb_uid(text):
    # Facebook UID শুধু সংখ্যা হয়, সাধারণত ৫-২০ ডিজিটের মধ্যে
    text = text.strip()
    if not text.isdigit():
        return False
...
def _handle_menu_inner(message, user_id):
    ...
    if text == '❌ বাতিল':
        update_user_data(user_id, {"state": None, "temp_fb_uid": None})
    ...
    if current_state == 'WAITING_FOR_FB_UID':
        if not is_valid_fb_uid(text):
            bot.send_message(message.chat.id, "😔 দুঃখিত, এটি সঠিক UID নয়! অনুগ্রহ করে সঠিক UID (শুধু সংখ্যা) দিন।", reply_markup=cancel_keyboard(), parse_mode="Markdown")
            return

        # UID টা সাময়িকভাবে সেভ রাখা হলো, এরপর কুকিজ চাওয়া হবে
        update_user_data(user_id, {
            "temp_fb_uid": text,
            "state": 'WAITING_FOR_FB_COOKIES'
        })
        bot.send_message(message.chat.id, "📝 **এবার আপনার ফেসবুক অ্যাকাউন্টের কুকিজটি দিন:** 🎯", reply_markup=cancel_keyboard(), parse_mode="Markdown")
        return

    elif current_state == 'WAITING_FOR_FB_COOKIES':
        ...
        fb_uid = user_data.get('temp_fb_uid', 'N/A')
        combined_data = f"UID: {fb_uid}\nCookies: {text}"

        update_user_data(user_id, {"pending_tasks": user_data['pending_tasks'] + 1})

        # টাস্কটি (UID + কুকিজ দুটোই) সরাসরি ডাটাবেজে সেভ করা হলো
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute('''
            INSERT INTO submitted_tasks (user_id, task_type, acc_name, acc_pass, task_data)
            VALUES (%s, %s, %s, %s, %s)
        ''', (user_id, task_type, acc_name, acc_pass, combined_data))
        conn.commit()
        cur.close()
        release_db_connection(conn)
    ...
    if text == '📖 কাজ ▸':
        ...
        msg_text = (
            ...
            f"🔐 **Password:** {password}\n\n"
            f"📘 উপরের তথ্য দিয়ে অ্যাকাউন্ট খুলে নিচে **Send UID** বাটনে চাপ দিন 🤪"
        )
        ...
        update_user_data(user_id, {"state": 'WAITING_FOR_FB_UID'})
        bot.send_message(message.chat.id, "📝 **আপনার ফেসবুক অ্যাকাউন্টের UID টি দিন:** 🎯", reply_markup=cancel_keyboard(), parse_mode="Markdown")
